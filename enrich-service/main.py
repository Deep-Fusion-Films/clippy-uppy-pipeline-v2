...
from gemini_flash import call_gemini_flash

PROJECT_ID = os.getenv("GCP_PROJECT", "local-project")
NEXT_TOPIC = os.getenv("STORE_TOPIC", "pipeline.v2.store")

app = FastAPI(title="Enrich Service (Flash)", version="1.0.0")
...
async def enrich_endpoint(request: Request):
    ...
    # Call Gemini Flash
    clip_metadata = call_gemini_flash(transcript, local_frames)

    # Cleanup
    shutil.rmtree(frames_dir, ignore_errors=True)

    # Publish to next stage (store-service)
    next_message = payload.model_dump()
    next_message["enriched_metadata"] = clip_metadata.model_dump()

    message_id = publish(NEXT_TOPIC, PROJECT_ID, next_message)

    log("Published enriched metadata", message_id=message_id)
